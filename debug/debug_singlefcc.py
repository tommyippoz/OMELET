...
SCORES_FILE = "test_single_fccs.csv"
...
def get_misclassification_detectors(classifier, data_dict: dict, detectors_dict: dict) -> list:
    ...
    for sg in [SPROUTGroup.UM1, SPROUTGroup.UM3, SPROUTGroup.UM4, SPROUTGroup.UM8]:
        ...
        if dataset_file.endswith(".csv"):
            ...
            for alr in get_alrs():
                ...
                if not (exp_hist is not None and ((exp_hist['dataset_tag'] == dataset_name) &
                    ...
                    # Printing data of candidate FCCs
                    val_df = pandas.DataFrame(data=data_dict["x_val"], columns=data_dict["feature_names"])
                    val_df["true_label"] = data_dict["y_val"]
                    test_df = pandas.DataFrame(data=data_dict["x_test"], columns=data_dict["feature_names"])
                    test_df["true_label"] = data_dict["y_test"]
                    for fcc in suitable_fccs:
                        fcc_name = fcc.get_name()
                        val_df[fcc_name + "_pred"] = fcc.predict(data_dict["x_val"])
                        test_df[fcc_name + "_pred"] = fcc.predict(data_dict["x_test"])
                    val_df.to_csv(dataset_name + "(" + str(alr) + ")_VAL_FCC_PREDS.csv", index=False)
                    test_df.to_csv(dataset_name + "(" + str(alr) + ")_TEST_FCC_PREDS.csv", index=False)
